...
class FasterRCNN(nn.Module):
    ...
    def build_backbone(self, gpu_id):
        backbone = torchvision.models.vgg16(weights="VGG16_Weights.DEFAULT").features[:30].cuda(gpu_id)
    ...
    def build_FastRCNN(self, FastRCNN_config, gpu_id):
        classifier = list(torchvision.models.vgg16(weights="VGG16_Weights.DEFAULT").classifier)
    ...
    def build_RPN(self, RPN_config, gpu_id):
        anchor_generator = rpn_for_ilod.AnchorGenerator(RPN_config['anchor_scale'], RPN_config['anchor_aspect_ratio'], 
                                               RPN_config['downsample'], gpu_id)
        rpn_head = rpn_for_ilod.RPNHead(RPN_config['in_channels'], RPN_config['num_anchors'], gpu_id)
        RPN = rpn_for_ilod.RegionProposalNetwork(anchor_generator, rpn_head, 
    ...
    def forward(self, images, gt_labels=None, gt_bboxs=None):
        ...
        proposals, objectness, pred_bbox_deltas = self.RPN(images, features, gt_labels, gt_bboxs)
        ...
        return  roi_cls_loss, roi_loc_loss, labels, scores, detections