...
class FasterRCNN():
    ...
    def train(self, train_loader, test_loader):
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(optimizer, self.TRAIN_config['milestones'],self.TRAIN_config['gamma'])
        
        self.model.train() # train mode 전환
        for epoch in range(self.TRAIN_config['epochs']):
            print('Epoch {} Started...'.format(epoch+1))
            for i, (images, labels, bboxs) in tqdm(enumerate(train_loader),desc = "Training"):
            ...
            if epoch % self.TRAIN_config['epoch_freq'] == 0:
                ...
                if (mAP > self.best_mAP) and (self.TRAIN_config['save']):
                    ...
                    torch.save(self.model.state_dict(), 
                               self.TRAIN_config['SAVE_PATH'] + 'epoch_{}.pt'.format(str(epoch+1).zfill(3)+"_mAP_"+str(self.best_mAP)))
    ...
    def val(self, test_loader):
        ...
        with torch.no_grad():
            ...
            gt_labels, gt_bboxs = [], []
            for images, gt_labels_, gt_bboxs_ in tqdm(test_loader, desc = "Validation"):
