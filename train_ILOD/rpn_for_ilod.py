...
class AnchorGenerator(nn.Module):
    ...
    def generate_images_anchors(self, features, downsample):
        ...
        for f_h, f_w in features_size:
            ...
            grid_y, grid_x = grid_y.reshape(-1), grid_x.reshape(-1)                 # == grid_y.reshape(1,-1), grid_x.reshape(1,-1) => 1차원 배열
...
class RegionProposalNetwork(nn.Module):
    ...
    def filter_proposals(self, images, objectness, proposals):
        ...
        for img, objectness_prob_per_img, proposals_per_img in zip(images, objectness_prob, proposals):
            ...
            # NMS
            keep_idx = torchvision.ops.nms(proposals_per_img, objectness_prob_per_img, self.nms_thresh) # iou > nms_thresh 인 것만 남김
            objectness_prob_per_img, proposals_per_img = objectness_prob_per_img[keep_idx], proposals_per_img[keep_idx]
            
            # sort by objectness and select top-n
            top_idx = torch.argsort(objectness_prob_per_img, descending=True)[:self.top_n()]    # top_n개만 남겨서 region proposal
    ...
    def forward(self, images, features, gt_labels=None, gt_bboxs=None):
        anchors = self.anchor_generator(features.detach())          # detach함수를통해 backpropagation 멈춤
        objectness, pred_bbox_deltas = self.rpn_head(features)
        objectness, pred_bbox_deltas = self.convert(objectness, pred_bbox_deltas)   # objectness_score, bbox_deltas => (1),(4)
        
        proposals = self.box_coder.decode(pred_bbox_deltas.detach(), anchors)           
        filtered_proposals = self.filter_proposals(images, objectness.detach(), proposals)  # region proposal
        ...
        return filtered_proposals, objectness, pred_bbox_deltas
    def top_n(self):
